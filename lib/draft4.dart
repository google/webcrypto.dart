...
///   * Why does platform libraries have few examples?
///     Are we trying to avoid duplicating information? Or is it simply because
///     nobody thought it useful to explain how to use:
///     `Utf8Decoder.startChunkedConversion()` and `Utf8Codec.decodeStream()`
///     As well as cross linking these?
///     Not a critism, simply asking: Am I likely to examples repeatitive docs
///     rejected in a review?
///     Maybe look at docs for [RsassaPkcs1V15PrivateKey] in this document.
///     Is that too much, is there no such thing? :)
///   * Can I reference pub packages from examples in documentation?
///     (useful for cases where I need utility methods not present in the sdk)
///      - PEM parsing / formatting
///      - Encoding/decoding as hex (package:convert)
///      - Converting Streams to buffers
///      - Slicing byte arrays (Uint8List)
///      (yes, stuff like this is missing in `dart:typed_data`)
///
///   Remarks on the SDK: (just asking, hehe)
///   * There is no singleton streams, there's `Stream.fromFuturè<T>` could we
///     change that to `Stream.fromFuture(FutureOr<T>)`?
///   * Can we add utility methods for slicing `TypedData` and converting
///     `Stream<List<int>>` to `Uint8List`. Afaik the SDK doesn't even have
